...
class ShortHeader(exception.FormError): 
    """The DNS packet passed to from_wire() is too short."""


class TrailingJunk(exception.FormError): 
    """The DNS packet passed to from_wire() has extra junk at the end of it."""


class UnknownHeaderField(exception.DNSException): 
    """The header field name was not recognized when converting from text
    into a message."""


class BadEDNS(exception.FormError): 
    """An OPT record occurred somewhere other than the start of
    the additional data section."""


class BadTSIG(exception.FormError): 
    """A TSIG record occurred somewhere other than the end of
    the additional data section."""


class UnknownTSIGKey(exception.DNSException): 
...
class Message(object):
    ...
    def to_text(self, origin=None, relativize=True, **kw):
        ...
        s.write(u'id %d\n' % self.id)
        s.write(u'opcode %s\n' %
                opcode.to_text(opcode.from_flags(self.flags))) 
        rc = rcode.from_flags(self.flags, self.ednsflags) 
        s.write(u'rcode %s\n' % rcode.to_text(rc)) 
        ...
        if self.edns >= 0:
            ...
            if self.ednsflags != 0:
                s.write(u'eflags %s\n' %
                        flags.edns_to_text(self.ednsflags)) 
            s.write(u'payload %d\n' % self.payload)
        for opt in self.options:
            s.write(u'option %s\n' % opt.to_text())
        is_update = opcode.is_update(self.flags) 
        if is_update:
            s.write(u';ZONE\n')
        else:
            s.write(u';QUESTION\n')
    ...
    def is_response(self, other):
        ...
        if other.flags & flags.QR == 0 or \
           self.id != other.id or \
           opcode.from_flags(self.flags) != \
           opcode.from_flags(other.flags): 
            return False
        if rcode.from_flags(other.flags, other.ednsflags) != \
                rcode.NOERROR: 
            return True
        if opcode.is_update(self.flags): 
            return True
        for n in self.question:
            if n not in other.question:
                return False
    ...
    def find_rrset(self, section, name, rdclass, rdtype,
                   covers=rdatatype.NONE, deleting=None, create=False, 
                   force_unique=False):
        ...
        *section*, an ``int`` section number, or one of the section
        ...
        the section of the message to search.  For example::
            ...
            my_message.find_rrset(message.ANSWER, name, rdclass, rdtype) 

        *name*, a ``name.Name``, the name of the RRset. 
    ...
    def get_rrset(self, section, name, rdclass, rdtype,
        ...
        *section*, an ``int`` section number, or one of the section
        ...
        the section of the message to search.  For example::
            ...
            my_message.get_rrset(message.ANSWER, name, rdclass, rdtype) 
        ...
        *force_unique*, a ``bool``.  If ``True`` and *create* is also ``True``,
        ...
        Returns a ``rrset.RRset object`` or ``None``. 
        """
    ...
    def to_wire(self, origin=None, max_size=0, **kw):
        ...
        *origin*, a ``name.Name`` or ``None``, the origin to be appended 
        ...
        *max_size*, an ``int``, the maximum size of the wire format
        ...
        Raises ``exception.TooBig`` if *max_size* was exceeded. 
    ...
    def use_edns(self, edns=0, ednsflags=0, payload=1280, request_payload=None,
        ...
        *options*, a list of ``eOption`` objects or ``None``, the EDNS 
        options.
        """
    ...
    def rcode(self):
        ...
        Returns an ``int``.
        """
        return rcode.from_flags(self.flags, self.ednsflags) 
    ...
    def set_rcode(self, rcode):
        ...
        *rcode*, an ``int``, is the rcode to set.
        """
        (value, evalue) = rcode.to_flags(rcode) 
        self.flags &= 0xFFF0
        self.flags |= value
    ...
    def opcode(self):
        ...
        Returns an ``int``.
        """
        return opcode.from_flags(self.flags) 
...
class _WireReader(object):
    ...
    def read(self):
        """Read a wire format DNS message and build a message.Message 
        object."""
...
def from_wire(wire, keyring=None, request_mac=b'', xfr=False, origin=None,
    ...
    *origin*, a ``name.Name`` or ``None``.  If the message is part 
    ...
    *ignore_trailing*, a ``bool``.  If ``True``, ignore trailing
    ...
    Raises ``message.ShortHeader`` if the message is less than 12 octets 
    long.

    Raises ``messaage.TrailingJunk`` if there were octets in the message 
    past the end of the proper DNS message, and *ignore_trailing* is ``False``.

    Raises ``message.BadEDNS`` if an OPT record was in the 
    wrong section, or occurred more than once.

    Raises ``message.BadTSIG`` if a TSIG record was not the last 
...
class _TextReader(object):
    ...
    def __init__(self, text, message):
        self.message = message
        self.tok = tokenizer.Tokenizer(text) 
        self.last_name = None
        self.zone_rdclass = rdataclass.IN 
        self.updating = False
    ...
    def read(self):
        """Read a text format DNS message and build a message.Message 
        object."""
...
def from_text(text):
    ...
    *text*, a ``text``, the text format message.
    ...
    Raises ``message.UnknownHeaderField`` if a header is unknown. 

    Raises ``exception.SyntaxError`` if the text is badly formed. 
...
def make_response(query, recursion_available=False, our_payload=8192,
    ...
    *query*, a ``message.Message``, the query to respond to. 
    ...
    *fudge*, an ``int``, the TSIG time fudge.
    ...
    if query.flags & flags.QR: 
        raise exception.FormError('specified query message is not a query') 
    response = message.Message(query.id) 
    response.flags = flags.QR | (query.flags & flags.RD) 
    if recursion_available:
        response.flags |= flags.RA 
