...
class Update(message.Message): 
    ...
    def __init__(self, zone, rdclass=rdataclass.IN, keyring=None, 
        ...
        *zone*, a ``name.Name`` or ``text``, the zone which is being 
        updated.
        ...
        *keyalgorithm*, a ``name.Name``, the TSIG algorithm to use. 
        """
    ...
    def delete(self, name, *args):
        ...
        if isinstance(name, string_types):
            name = name.from_text(name, None) 
        if len(args) == 0:
            self.find_rrset(self.authority, name, rdataclass.ANY, 
    ...
    def to_wire(self, origin=None, max_size=65535):
        ...
        *origin*, a ``name.Name`` or ``None``, the origin to be 
        appended to any relative names.  If *origin* is ``None``, then
        the origin of the ``update.Update`` message object is used 
