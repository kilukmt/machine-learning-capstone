...
class UnsupportedAlgorithm(exception.DNSException): 
...
def make_ds(name, key, algorithm, origin=None):
    ...
    *origin* is a ``name.Name`` and will be used as the origin 
    if *key* is a relative name.

    Returns a ``rdtypes.ANY.DS``. 
    """

    if algorithm.upper() == 'SHA1':
        dsalg = 1
        hash = hash.hashes['SHA1']() 
...
def _find_candidate_keys(keys, rrsig):
    ...
    if isinstance(value, node.Node): 
        try:
            rdataset = value.find_rdataset(rdataclass.IN, 
                                           rdatatype.DNSKEY) 
        except KeyError:
            return None
    else:
...
def _make_hash(algorithm):
    if _is_md5(algorithm):
        return hash.hashes['MD5']() 
    if _is_sha1(algorithm):
        return hash.hashes['SHA1']() 
    if _is_sha256(algorithm):
        return hash.hashes['SHA256']() 
    if _is_sha384(algorithm):
        return hash.hashes['SHA384']() 
    if _is_sha512(algorithm):
        return hash.hashes['SHA512']() 
...
def _validate_rrsig(rrset, rrsig, keys, origin=None, now=None):
    ...
    *rrset* is the RRset to validate.  It can be a ``rrset.RRset`` or 
    a ``(name.Name, rdataset.Rdataset)`` tuple. 

    *rrsig* is a ``rrset.Rdata``, the signature to validate. 

    *keys* is the key dictionary, used to find the DNSKEY associated with
    a given name.  The dictionary is keyed by a ``name.Name``, and has 
    ``node.Node`` or ``rdataset.Rdataset`` values. 

    *origin* is a ``name.Name``, the origin to use for relative names. 
...
def _validate(rrset, rrsigset, keys, origin=None, now=None):
    ...
    *rrsigset* is the signature RRset to be validated.  It can be a
    ``rrset.RRset`` or a ``(name.Name, rdataset.Rdataset)`` tuple. 
