...
class TransferError(exception.DNSException): 
    """A zone transfer response got a non-zero rcode."""

    def __init__(self, rcode):
        message = 'Zone transfer error: %s' % rcode.to_text(rcode) 
...
def send_udp(sock, what, destination, expiration=None):
    ...
    *what*, a ``binary`` or ``message.Message``, the message to send. 
    ...
    *expiration*, a ``float`` or ``None``, the absolute time at which
    ...
    if isinstance(what, message.Message): 
        what = what.to_wire()
    ...
    *request_mac*, a ``binary``, the MAC of the request (for TSIG).
    ...
    while 1:
        ...
        if _addresses_equal(sock.family, from_address, destination) or \
           (inet.is_multicast(destination[0]) and 
    ...
    r = message.from_wire(wire, keyring=keyring, request_mac=request_mac, 
...
def udp(q, where, timeout=None, port=53, af=None, source=None, source_port=0,
    ...
    *q*, a ``message.message``, the query to send 
...
def xfr(where, zone, rdtype=rdatatype.AXFR, rdclass=rdataclass.IN, 
    ...
    *zone*, a ``name.Name`` or ``text``, the name of the zone to transfer. 

    *rdtype*, an ``int`` or ``text``, the type of zone transfer.  The
    default is ``rdatatype.AXFR``.  ``rdatatype.IXFR`` can be 
    used to do an incremental transfer instead.

    *rdclass*, an ``int`` or ``text``, the class of the zone transfer.
    The default is ``rdataclass.IN``. 
    ...
    *keyname*, a ``name.Name`` or ``text``, the name of the TSIG 
    key to use.

    *relativize*, a ``bool``.  If ``True``, all names in the zone will be
    relativized to the zone origin.  It is essential that the
    relativize setting matches the one specified to
    ``zone.from_xfr()`` if using this generator to make a zone. 
    ...
    *keyalgorithm*, a ``name.Name`` or ``text``, the TSIG algorithm to use. 

    Raises on errors, and so does the generator.

    Returns a generator of ``message.Message`` objects. 
    """
