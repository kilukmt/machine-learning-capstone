...
public_enum_domain = name.from_text('e164.arpa.') 
...
def from_e164(text, origin=public_enum_domain):
    ...
    *origin*, a ``name.Name``, the domain in which the number 
...
def to_e164(name, origin=public_enum_domain, want_plus_prefix=True):
    ...
    *name* is a ``name.Name``, the ENUM domain name. 

    *origin* is a ``name.Name``, a domain containing the ENUM 
...
def query(number, domains, resolver=None):
    ...
    *domains* is an iterable containing ``name.Name`` values. 

    *resolver*, a ``resolver.Resolver``, is the resolver to use.  If 
    ``None``, the default resolver is used.
    """

    if resolver is None:
        resolver = resolver.get_default_resolver() 
    e_nx = resolver.NXDOMAIN() 
