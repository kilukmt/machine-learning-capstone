...
class BadTime(exception.DNSException): 

    """The current time is not within the TSIG's validity time."""


class BadSignature(exception.DNSException): 

    """The TSIG signature fails to verify."""


class PeerError(exception.DNSException): 
...
def sign(wire, keyname, secret, time, fudge, original_id, error,
    ...
    if first:
        ...
        ctx.update(struct.pack('!H', rdataclass.ANY)) 
...
def get_algorithm(algorithm):
    ...
    try:
        return (algorithm.to_digestable(), hash.hashes[_hashes[algorithm]]) 
    except KeyError:
